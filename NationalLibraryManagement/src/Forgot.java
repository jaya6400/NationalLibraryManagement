...
public class Forgot extends JFrame implements ActionListener{
	...
	contentPane = new JPanel(); //work as a div
	setContentPane(contentPane); // adding to the frame
    contentPane.setBackground(Color.WHITE);
	contentPane.setLayout(null); //border layout = default, flow layout, grid layout
	...
	t2 = new JTextField();
	t2.setEditable(false); //to disable to write
    ...
    public void actionPerformed(ActionEvent ae){
		...
		ResultSet rs = st.executeQuery(); //it has two functions getString and setString

		while (rs.next()) {
                    t2.setText(rs.getString("name")); //to select the column name
